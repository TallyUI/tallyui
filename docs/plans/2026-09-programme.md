...
| 11 | **TallyUI half done 2026-09-25 (ADR-062: discounts are pre-tax and allocated per line, and a discounted `order.create` is version 2; `finalize` keeps rejecting discounts); the plugin half is queued (medusapos line adjustments, then the guard's removal).** **Discount contract:** an ADR plus a version bump, fixing the after-tax order-discount basis | A discounted sale lands in Medusa within ≤ 1 minor unit of the POS total |
...
| 28 | **Done 2026-09-25 (per-variant `trackInventory`, `outOfStockThreshold` and `useGlobalOutOfStockThreshold` ride in the Vendure product schema v1, with `vendureGlobalStockSettings` for the channel defaults; `getStock`, `getStockStatus` and `getStockQuantity` apply tracking, the threshold and Vendure's own saleable rule across every variant).** ~~**Vendure stock traits: three gaps**~~ (added 2026-09-24, from the #45 stock review) | Tests show each fixed: (a) a variant with `trackInventory` `FALSE`, or `INHERIT` with the global setting off, is always in stock rather than out of stock; (b) `outOfStockThreshold` (and the global threshold) is subtracted from available stock; (c) `getStockStatus`/`getStockQuantity` aggregate every variant as `getStock` does, rather than reading variant 0 |
| 29 | **Done 2026-09-25 (the high-water mark and both skew probes use their own minimal `id updatedAt` query; the page query is unchanged).** ~~**Vendure pull: lighter mark and probe reads.**~~ The high-water mark and the two skew probes fetch the full product query; they need only `id` and `updatedAt` (added 2026-09-24, from the #45 review) | A request-body test shows the mark and probe queries select only `id updatedAt` |
| 30 | **Done 2026-09-25 (a mid-pass checkpoint in the shape saved before #45's pass-state fields is recognised and normalised into a clean restart of the pass from offset 0).** ~~**Vendure pull: recover from a checkpoint stored mid-pass by the old code.**~~ Such a checkpoint wastes three requests per sync until the next catalogue write (added 2026-09-24, from the #45 review) | A replication test that starts from an old-shape mid-pass checkpoint makes no more than one extra request per sync after the first |
| 31 | **Done 2026-09-25 (the probe re-reads the mark once before throwing: a deletion between the mark read and the probe now gives a clean pass with no error; a persistent mismatch still throws).** ~~**Vendure pull: the guard throws once if the newest product is deleted between the mark read and the probe.**~~ This is harmless, because RxDB retries (added 2026-09-24, from the #45 review) | A test shows a single retried error and then a clean pass; optionally, the probe re-reads the mark once before throwing |
...
| 39 | **The WooCommerce connector projects REST rows onto its schema.** The WooCommerce pull stores REST rows unchanged (`{ ...p, _deleted: false }`). Real WooCommerce responses carry fields the schema rejects, such as `weight`, `date_created`, `date_created_gmt` and `date_modified`, so a live WooCommerce sync would fail validation. #82 made only the mock API's rows match. Not urgent: WooCommerce is not an app target yet (added 2026-09-24, from the #82 review) | The WooCommerce pull maps each REST row through a `toDocument` projection onto the schema's fields, as the Medusa connector does; a test feeds a real-shaped WooCommerce REST row, including `weight` and the `date_*` fields, through the pull into a validating collection |
| 40 | **WooCommerce prices group thousands.** The demo shows a WooCommerce product at `$1299.00` and the same product under Medusa at `$1,299.00`: the WooCommerce price trait's formatting leaves out the thousands separator. Not urgent (added 2026-09-24, from the #82 screenshots) | WooCommerce and Medusa format the same amount identically in the same locale; a trait test covers an amount of 1,000 or more |
| 41 | **Done 2026-09-24 (`pull.seedCheckpoint`: on a fresh install `combinePullAdapters` seeds the Medusa and Vendure variant feeds at their variant mark before any feed runs; an upgrade keeps the healing pass).** ~~**A fresh Medusa install downloads the catalogue once, not twice.**~~ On a fresh install, the variant feed's first pass re-delivers every product the product feed has just delivered, which doubles the first download to no purpose: 31 MB and 46 s against 15 MB and 31 s at 2,005 products (measured on medusa-dev after #81). A price edit made during the product feed's first pass must still arrive (added 2026-09-24, from the #81 review) | With no checkpoint, the variant feed's cursor starts from a variant mark read before the product feed's first pass begins, so the first sync makes one catalogue pass; a real-loop test shows a fresh install downloads each product once, and a variant price edited during the product pass still arrives |
| 42 | **The Medusa ordering guard is proven like Vendure's.** Vendure's scenario 7 holds the variant feed's parent fetch while a product rename lands, and shows the rename is not overwritten by the older parent. Medusa's variant feed (#81) has no twin of it (added 2026-09-24, from the #81 review) | A Medusa real-loop test holds the variant feed's `fetchByIds` while a rename lands on the same product, then releases it; the stored product ends with the rename and the new price |
| 43 | **Done 2026-09-25 (`ReconcileFeedEntry.refreshOnly`: the fingerprint runner enqueues refresh-only entries, which the feed skips rather than tombstones when the product is missing; `enqueue`'s id-based merge never downgrades a deletable id-reconcile entry to refresh-only).** ~~**A fingerprint mismatch never deletes.**~~ When the fingerprint reconcile (#85) queues a product and the reconcile feed's re-fetch then finds it missing, the feed writes a tombstone, bypassing the id reconcile's mass-deletion brake. This needs two failures at once (a price drift and a product that vanished), so the risk is low, but deletions are the id reconcile's job, and only it applies the brake (added 2026-09-24, from the #85 review) | Entries queued by the fingerprint path are skipped, not tombstoned, when the re-fetch finds no product; tombstones still come only from the id reconcile; a test queues a fingerprint entry for a product the server no longer has and shows the local document survives, while an id-reconcile entry for the same case still deletes |
| 44 | **Done 2026-09-25 (Medusa products schema version 1 declares `variants[].calculated_price`; a connector schema bump is drop-and-resync by rule: `createTallyDatabase` supplies `v => null` strategies and `startReplication`'s identifier gains `-v<version>` above version 0, ADR-060 amendment 9).** ~~**Medusa `calculated_price` moves into the schema.**~~ D2b stores it as an extra variant property, untyped in the JSON schema, to avoid a version bump. The SQLite engine switch (a cold resync under a new database name) makes the bump cheap | The Medusa variant item schema declares `calculated_price` (object or null) in the same release as the engine switch's new database name; drop strategies come from `connectorCollection`, and the versioned replication identifier resets the checkpoint (ADR-060 amendment 9) |
| 45 | **A partly listed product does not re-queue every 30 minutes.** When the store API lists a product but omits one of its variants (for example, a variant outside the sales channel), the local document's calculated-price fingerprint has that variant as `:-` while the store's fingerprint has no entry for it. The two never match, so the product re-queues on every 30-minute pass. The live run on medusa-dev found 0 such products (added 2026-09-24, from the #92 review) | The calculated-price fingerprint covers only the variants the store reports, on both sides, or the runner skips an entry after N identical re-queues; a test lists a product with one variant missing and shows one re-delivery, then no re-queue on the next pass |
| 46 | **Done 2026-09-25 (`FingerprintReconcileState` gains `lastResultAt`/`lastErrorAt`, stamped from an injectable `now`; the exported `isFingerprintResultCurrent(state)` tells whether the kept `lastResult` is newer than the last failure).** ~~**A failed fingerprint pass clears the stale result.**~~ After a failed pass, `state$` keeps the previous `lastResult`, including its `unreported` count, next to the new `lastError`, so the app can show an old "N not sellable" as current. This predates #92 (added 2026-09-24, from the #92 review) | A failed pass either clears `lastResult` or marks it stale (for example `lastResultAt` and `lastErrorAt`, so the app can tell which is newer); a test fails a pass after a good one and shows the app can tell the count is stale |
| 47 | **Vendure live tests that write don't race.** `reconcile/stock.live` (setStock) and `reconcile/ids.live` (it creates and deletes a product) both write to vendure-dev. In one `vitest run connectors/vendure` with 2 workers, `stock.live` picked the product `ids.live` was deleting and failed with `ENTITY_NOT_FOUND`. Each passes alone (added 2026-09-25, from the #110 review) | The writing Vendure live tests run serially (`describe.sequential` or a `fileParallelism: false` project for `*.live.test.ts`), or `stock.live` picks a product `ids.live` never touches; a whole-connector live run passes twice in a row |
| 48 | **One receipt renderer, one template language.** When WCPOS's receipt schema and renderer are copied in (ADR-032), the sale receipt converges onto the same receipt-template envelope that the register's X and Z closure documents use (registers job b2). `ReceiptData` (`packages/pos/src/receipt`) becomes an input to that envelope, not a second document shape (added 2026-09-25, the Front desk) | The sale receipt, the X report and the Z report render through the one copied renderer from the one envelope shape; no TallyUI document shape exists beside it |
