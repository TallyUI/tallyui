...
The app also creates `pos_orders`, the local store of finalized sales, with `posOrderCollection()`. Its migration strategies keep every order through a schema bump, and the collection holds sales not yet sent, so never create it from `posOrderSchema` alone (RxDB refuses a schema above version 0 without its strategies), and never with `connectorCollection`, whose bumps drop documents.

```ts
import { createTallyDatabase, connectorCollection, getStorage } from '@tallyui/database';
import { posOrderCollection } from '@tallyui/pos';
...
await db.addCollections({
  device_settings: connectorCollection(deviceSettingsSchema),
  pos_orders: posOrderCollection(),
});
```
