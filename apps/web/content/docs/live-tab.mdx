...
`startLiveTab` returns a `state$` observable of `'acquiring' | 'live' | 'parked' | 'blocked'`, a `takeOver()` you call on start (and again from the parked screen's "Use here" button), and a `stop()` for sign-out or unmount.
...
function App() {
  ...
  return (
    <>
      ...
      {(state === 'acquiring' || state === 'live') && <POS />}
    </>
  );
}
```
