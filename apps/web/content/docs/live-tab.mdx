...
const liveTab = startLiveTab({
  ...
  onPark: async () => {
    ...
    // 2. Then terminate the worker, which releases the opfs-sahpool access handles.
    dbWorker.terminate();
...
- **Then terminate the worker.** Terminating releases the opfs-sahpool access handles.
