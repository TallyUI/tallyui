...
WooCommerce calls it `name`, Medusa calls it `title`. WooCommerce puts price as a string on the product, Medusa stores it as a number in major units nested inside variants. Images use `src` in one and `url` in the other.
...
interface TallyConnector {
  ...
  replication: {        // Pull-only sync via RxDB replication
...
Tally UI uses RxDB's built-in replication protocol to sync the local database from remote APIs. Each connector implements a `ReplicationAdapter` with a checkpoint-based pull handler. Catalogue replication is pull-only; POS-created data such as orders will go through a command outbox.

The replication layer handles:
- **Checkpoint tracking** -- only fetches data that changed since the last sync
- **Pull-only catalogue sync** -- pulls remote changes; the POS never writes products
...
Storage is platform-aware. The `getStorage()` factory returns the right
adapter for Node/SSR/tests automatically. On the web and React Native it
throws with guidance instead: both need a storage built and bundled outside
`@tallyui/database` (ADR-061, ADR-031).

| Platform | Storage | Package |
|----------|---------|---------|
| Web browser | RxDB Premium SQLite-wasm (opfs-sahpool), one live tab | `@tallyui/storage-sqlite` |
| React Native | RxDB Premium SQLite (via expo-sqlite) | `@tallyui/storage-sqlite` |
...
including the web storage's breaking change from Dexie.
...
| `@tallyui/storage-sqlite` | RxDB Premium SQLite storage: expo-sqlite on native, sqlite-wasm (opfs-sahpool) on the web |
