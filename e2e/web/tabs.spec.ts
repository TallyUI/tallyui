...
test.describe('Tabs primitive', () => {
  ...
  test('clicking a tab changes content', async ({ page }) => {
    ...
    await expect(page.getByText('First Tab')).toBeVisible();
    await expect(page.getByText('Second Tab')).not.toBeVisible();

    // Click second tab trigger
    await page.getByRole('tab', { name: 'Tab 2' }).click();
    await expect(page.getByText('Second Tab')).toBeVisible();
    await expect(page.getByText('First Tab')).not.toBeVisible();

    // Click third tab trigger
    await page.getByRole('tab', { name: 'Tab 3' }).click();
    await expect(page.getByText('Third Tab')).toBeVisible();
    await expect(page.getByText('Second Tab')).not.toBeVisible();
  });
